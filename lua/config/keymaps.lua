local keymaps = {
  n = {
    ...
    ["<leader>tw"] = {
      "<Cmd>TroubleToggle workleader_diagnostics<CR>",
      "toggle the `trouble` panel for current workleader",
    },
    ["<leader>td"] = {
      "<Cmd>TroubleToggle document_diagnostics<CR>",
      "toggle the `trouble` panel for current document",
    },
    ["<leader>tn"] = {
      "<Cmd>lua require('trouble').next({ skip_groups = true, jump = true})<CR>",
      "move to the next result in `Trouble`",
    },
    ["<leader>tb"] = {
      "<Cmd>lua require('trouble').previous({ skip_groups=true, jump = true })<CR>",
      "move to the previous result in `Trouble`",
    },
    ...
    ["<leader>dp"] = {
      ...
      "move to the current line previous lsp diagnostic",
    },
    ...
    ["<leader>sr"] = {
      ...
      "insert the search and replace command",
    },
  },
}

for mode, maps in pairs(keymaps) do
  for map, keymap in pairs(maps) do
    local command = keymap[1]
    local description = keymap[2]
